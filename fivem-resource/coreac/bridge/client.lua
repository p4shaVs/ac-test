-- =============================================================================
-- CoreAC Panel Bridge — Client Side
-- Modules client dosyalarının ihtiyaç duyduğu tüm CoreAC client global'leri
-- =============================================================================

-- CAC = client tarafı global (client/core.lua'dan gelir)
CAC = CAC or {}
...
-- CAC.report → TriggerServerEvent('coreac:report', ...) zinciri üzerinden gider.
-- ---------------------------------------------------------------------------
...
-- Araç modifier değerleri (vehicleSpeed.lua okur). Aşağıdaki durum döngüsü
-- sürücüyken bunları FiveM'in CFX getter native'lerinden doldurur:
--   GetVehicleTopSpeedModifier / GetVehicleCheatPowerIncrease /
--   GetVehicleGravityAmount.
-- Eskiden hiç doldurulmuyordu (sabit kalıyordu) → araç hız/handling hilesi
-- tespiti hiçbir koşulda tetiklenemiyordu. Başlangıç değerleri oyunun
-- varsayılanlarıdır (değiştirilmemiş araç).
CoreAC.vehicleTopSpeedModifier   = -1.0
CoreAC.vehicleCheatPowerIncrease = 1.0
CoreAC.vehicleGravityAmount      = 9.8

-- Native'ler eski bir FiveM build'inde yoksa (nil) okuma atlanır; kontrol
-- sessizce pasif kalır — sahte değer üretip false tespit ATMAZ.
local nGetTopSpeedMod = GetVehicleTopSpeedModifier
local nGetCheatPower  = GetVehicleCheatPowerIncrease
local nGetGravity     = GetVehicleGravityAmount
CoreAC.vehicleModifierNativesAvailable = (nGetTopSpeedMod ~= nil and nGetCheatPower ~= nil and nGetGravity ~= nil)
    ...
    while true do
        ...
        if ready then
            ...
            if not CoreAC.playerSpawned and now - readySince >= need then
                ...
                if CAC and CAC.markRevive then CAC.markRevive() end
                TriggerEvent('coreac:pedChanged')
                -- Framework'süz sunucularda sunucuya ipucu (sunucu buna tek başına
                -- güvenmez; bkz. CAC.isInGame in server/main.lua).
                TriggerServerEvent('coreac:inGame')
...
CreateThread(function()
    while true do
        ...
        -- Araç modifier'ları: yalnızca sürücüyken ve native'ler mevcutsa.
        if veh ~= 0 and CoreAC.isPlayerDriver and CoreAC.vehicleModifierNativesAvailable then
            CoreAC.vehicleTopSpeedModifier   = nGetTopSpeedMod(veh) or -1.0
            CoreAC.vehicleCheatPowerIncrease = nGetCheatPower(veh) or 1.0
            CoreAC.vehicleGravityAmount      = nGetGravity(veh) or 9.8
        else
            CoreAC.vehicleTopSpeedModifier   = -1.0
            CoreAC.vehicleCheatPowerIncrease = 1.0
            CoreAC.vehicleGravityAmount      = 9.8
        ...
        while true do
            ...
            if CoreAC.playerSpawned and not CoreAC.isPlayerDead then
                ...
                if not ok and Config and Config.Debug then
                    print(('[CoreAC] Detection hata [%s]: %s'):format(name, tostring(err)))
                end
            end
        end
    end)
...
-- ---------------------------------------------------------------------------
RegisterNetEvent('coreac:rules', function(r)
...
-- ---------------------------------------------------------------------------
RegisterNetEvent('coreac:acConfig', function(rawAc)
...
CreateThread(function()
    Wait(2800)
    TriggerServerEvent('coreac:requestAcConfig')
end)
...
print('^2[CoreAC] Client bridge yuklendi.^7')
