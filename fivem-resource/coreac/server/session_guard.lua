...
AddEventHandler('playerConnecting', function(_, _, deferrals)
  ...
  if b.count >= 6 then
    b.count = 0
    CAC.log('WARN', 'session', ('Frequent reconnects: %s (6+ in 2 minutes)'):format(GetPlayerName(src) or ids.license))
    CAC.request('/detections', 'POST', {
      type = 'RECONNECT_SPAM', severity = 'LOW', origin = 'server',
      playerName = GetPlayerName(src) or 'Unknown', license = ids.license, details = {},
    }, nil)
  end
end)
...
AddEventHandler('chatMessage', function(_, _, _message)
  ...
  if b.count > 12 then
    CancelEvent()
    TriggerEvent('coreac:serverReport', src, 'CHAT_FLOOD', 'HIGH', {})
  end
end)
...
if Config.AllowedResources and #Config.AllowedResources > 0 then
  ...
  AddEventHandler('onResourceStart', function(resName)
    ...
    if not allowed[resName] then
      CAC.log('WARN', 'resource', ('Resource outside the allowlist started: %s'):format(resName))
    end
  end)
end
