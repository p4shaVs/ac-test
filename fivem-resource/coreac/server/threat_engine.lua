...
-- 'coreac:serverReport' köprüsünü (type='THREAT_SCORE') tetikler — böylece
...
function CAC.threatSignal(vid, dtype, details)
  vid = tonumber(vid)
  ...
  if t.score >= THRESH_BAN then
    ...
    scores[lic] = nil
    TriggerEvent('coreac:serverReport', vid, 'THREAT_SCORE', 'CRITICAL', { score = score, signals = #t.history })
  elseif t.score >= THRESH_KICK and not t.kickSent then
    ...
    t.kickSent = true
    TriggerEvent('coreac:serverReport', vid, 'THREAT_SCORE', 'HIGH', { score = math.floor(t.score), signals = #t.history })
  end
end
