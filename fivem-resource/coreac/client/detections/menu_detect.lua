...
--
-- NOT: CAC.report() kullanılır (CAC.threatSignal DEĞİL) — threatSignal
-- SUNUCU tarafı bir fonksiyondur, client'tan doğrudan çağrılamaz (FiveM'de
-- client/server ayrı Lua ortamlarıdır). CAC.report ağ üzerinden sunucuya
...
local frametimeStrike = CAC.strike(5, 30000)
local lastTick = GetGameTimer()

CreateThread(function()
  while true do
    Wait(1000)
    if CAC.rule('anti_cheat_menu', true) and CAC.active() then
      local marker = scanGlobalMarkers()
      if marker then
        CAC.report('CHEAT_MENU_SUSPECTED', 'LOW', { reason = 'global_marker', marker = marker }, 60000)
      end
      ...
      if dt > 1500 then
        if frametimeStrike:hit() then
          CAC.report('CHEAT_MENU_SUSPECTED', 'LOW', { reason = 'frametime_spike', dt = dt }, 30000)
        end
      end
    else
