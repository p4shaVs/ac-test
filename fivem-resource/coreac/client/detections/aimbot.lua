...
local snapStrike      = CAC.strike(3, 8000)
local precisionStrike = CAC.strike(2, 30000)
...
local function getAimTarget(ped)
  local aiming, ent = GetEntityPlayerIsFreeAimingAt(CAC.S.id)
...
local function evaluatePrecision(now)
  ...
  if mean <= 2.0 and stddev <= 1.0 and (now - precisionSince) >= 2500 then
    if precisionStrike:hit() then
      CAC.report('AIM_PRECISION_SUSPECTED', 'MEDIUM', {
        source = 'precision',
...
CreateThread(function()
  while true do
    local S = CAC.S
    if CAC.rule('anti_aimbot', true) and CAC.active()
