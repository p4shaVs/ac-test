...
--
-- Dosya: sunucuda resources/coreac/recordings/rec_<oyuncu>_<zaman>.json
...
RegisterCommand('acrec', function(_, args)
  local sub = args[1]
  if sub == 'on' then
    -- sunucudan yetki doğrulaması iste; server izinliyse coreac:rec:allow döner
    TriggerServerEvent('coreac:rec:auth')
  elseif sub == 'off' then
    if not recording then notify('~y~Kayıt zaten kapalı.'); return end
    recording = false
    TriggerServerEvent('coreac:rec:dump', buffer)
...
RegisterNetEvent('coreac:rec:allow', function()
  buffer = {}
  startAt = GetGameTimer()
  recording = true
  notify('~g~Kayıt BAŞLADI. Şimdi hileyi aç/dene. Bitince /acrec off')
end)
RegisterNetEvent('coreac:rec:deny', function()
