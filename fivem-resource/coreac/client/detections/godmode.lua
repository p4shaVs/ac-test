...
local flagStrike     = CAC.strike(6, 25000)
local lastVehExit    = 0
...
local function exemptState()
  ...
  if CAC.reviveGrace and CAC.reviveGrace() then return true end
  if CAC.tpGrace     and CAC.tpGrace()     then return true end
...
CreateThread(function()
  while true do
    Wait(2000)
    if CAC.rule('anti_invincibility', true) and CAC.active() and not exemptState() then
      local S       = CAC.S
      local flagged = false
