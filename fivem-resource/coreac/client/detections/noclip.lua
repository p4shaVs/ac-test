...
--
-- YENİ TASARIM: CAC.S zaten 200ms'de bir güncelleniyor; biz de 200ms'de bir
...
--
-- Hızlı sonuç sunucuya da hemen ulaşsın diye CAC.report zaten throttle'lı;
-- ayrıca client/main.lua'daki coreac:collState sinyali de hızlandırıldı ki
...
CreateThread(function()
  while true do
    Wait(200)
    if CAC.rule('anti_noclip', true) and CAC.active() and not CAC.tpGrace() then
      local S = CAC.S
      ...
      if ticksA >= 3 then
        ...
        CAC.report('NOCLIP', 'CRITICAL', { source = 'collision' })
      elseif ticksB >= 3 then
        ticksA, ticksB, ticksV = 0, 0, 0
        CAC.report('NOCLIP', 'CRITICAL', { source = 'underground' })
      elseif ticksV >= 3 then
        ticksA, ticksB, ticksV = 0, 0, 0
        CAC.report('VEHICLE_NOCLIP', 'CRITICAL', { source = 'vehicle_collision' })
      end
      ::cont::
    else
      ticksA, ticksB, ticksV = 0, 0, 0
