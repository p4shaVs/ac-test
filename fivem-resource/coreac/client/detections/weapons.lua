  ...
  while true do
    ...
    do
      ...
      if isFirearm and IsPedShooting(ped) and total > 0 then
        if CAC.rule('anti_infinite_ammo', true) and total == lastTotal then
          sameAmmo = sameAmmo + 1
          if sameAmmo >= 6 then sameAmmo = 0; CAC.report('INFINITE_AMMO', 'CRITICAL', {}) end
        else sameAmmo = 0 end
        if CAC.rule('anti_no_reload', true) and clip == lastClip and clip > 0 then
          sameClip = sameClip + 1
          if sameClip >= 6 then sameClip = 0; CAC.report('NO_RELOAD', 'CRITICAL', {}) end
        else sameClip = 0 end
...
CreateThread(function()
  ...
  while true do
    Wait(3000)
    local ped = CAC.S.ped
    if ped and CAC.rule('anti_give_all_weapons', true) and CAC.active() then
      local count = 0
      ...
      if lastCount ~= nil and (count - lastCount) >= 6 then
        CAC.report('GIVE_ALL_WEAPONS', 'CRITICAL', { before = lastCount, after = count })
      end
      lastCount = count
    else
      lastCount = nil
...
CreateThread(function()
  while true do
    ...
    local ped = CAC.S.ped
    if ped then
      for hash, action in pairs(CAC.WeaponBlacklist) do
        if HasPedGotWeapon(ped, hash, false) then
          if action == 'REMOVE' then
            RemoveWeaponFromPed(ped, hash)
            CAC.report('BLACKLIST_WEAPON', 'MEDIUM', { hash = hash })
          else
            TriggerServerEvent('coreac:weaponHit', hash)
          end
        end
      end
    end
