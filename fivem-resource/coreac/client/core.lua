-- CoreAC Anti-Cheat — client çekirdeği (paylaşılan durum + yardımcılar)
...
CAC = CAC or {}
CAC.Rules = CAC.Rules or {}

RegisterNetEvent('coreac:rules', function(r)
  if type(r) == 'table' then CAC.Rules = r end
end)
CreateThread(function() Wait(2500); TriggerServerEvent('coreac:requestRules') end)

--- Kural açık mı? (kural gelmediyse def)
function CAC.rule(key, def)
  local v = CAC.Rules[key]
...
local lastReport = {}
function CAC.report(dtype, severity, details, throttle)
...
-- ---------------------------------------------------------------------------
CAC.grace = { tp = 0, revive = 0, spectate = 0, spawn = GetGameTimer() + 30000 }
function CAC.markTp()
  CAC.grace.tp = GetGameTimer() + 10000
...
function CAC.markRevive() CAC.grace.revive = GetGameTimer() + 10000 end
function CAC.markSpectate(on)
  CAC.grace.spectate = on and (GetGameTimer() + 3600000) or 0
...
function CAC.spawnGuard() return GetGameTimer() < CAC.grace.spawn end
function CAC.tpGrace() return GetGameTimer() < CAC.grace.tp end
function CAC.reviveGrace() return GetGameTimer() < CAC.grace.revive end
function CAC.spectateGrace() return GetGameTimer() < CAC.grace.spectate end

-- Başka client script'lerinin kullandığı entegrasyon noktaları. Bir script
-- oyuncuyu kendisi taşıyacaksa (garaj/ev/iş) ÖNCE bunu çağırır; client
-- muafiyeti hemen başlar, sunucuya da ipucu gider (sunucu tarafındaki teleport
-- taraması da muaf olsun — sunucu bunu bütçeyle sınırlar, bkz. server/live.lua).
-- Olay adıyla çağrı klasör adından bağımsızdır (önerilen):
--     TriggerEvent('coreac:markTeleport')
local function scriptTeleport()
  CAC.markTp()
...
local function onRespawn()
  CAC.spawned = true
  CAC.grace.spawn = GetGameTimer() + 10000
  CAC.markTp()
  CAC.markRevive()
  replayBuf = {}
  ...
  TriggerServerEvent('coreac:respawnAnchor')
end
CAC.onRespawn = onRespawn

CAC.spawned = false
AddEventHandler('playerSpawned', onRespawn)
CreateThread(function()
  while not CAC.spawned do
    Wait(500)
...
function CAC.active() return CAC.spawned and not CAC.spawnGuard() end
RegisterNetEvent('coreac:grantTp', function() CAC.markTp() end)  -- sunucu admin tp
RegisterNetEvent('coreac:grantRevive', function() CAC.markRevive() end)

-- ---------------------------------------------------------------------------
-- Strike sistemi — tek fluke ban atmasın; N vuruş / pencere içinde tetikler
-- ---------------------------------------------------------------------------
function CAC.strike(needed, windowMs)
  return {
...
CAC.WeaponBlacklist = {}
RegisterNetEvent('coreac:weaponBlacklist', function(list)
  local m = {}
  for _, w in ipairs(list or {}) do m[w.hash] = w.action end
  CAC.WeaponBlacklist = m
end)
