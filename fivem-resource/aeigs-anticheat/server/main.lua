...
function Aeigs.eventLogEnabled()
...
RegisterNetEvent('aeigs:inGame', function()
  local src = source
  if Aeigs.noteEvent then Aeigs.noteEvent(src) end
  if Aeigs.eventLimited(src, 'inGame', 3, 60000) then return end
...
function Aeigs.isInGame(src, settleMs)
  src = tonumber(src)
...
-- görünür. Sunucu tarafından:
--     exports['aeigs-anticheat']:markTeleport(src)
--     TriggerEvent('aeigs:markTeleport', src)
-- Client tarafından:
--     exports['aeigs-anticheat']:markTeleport()
-- ---------------------------------------------------------------------------
function Aeigs.markTeleport(src)
  src = tonumber(src)
  if not src or src <= 0 or not GetPlayerName(src) then return end
  TriggerClientEvent('aeigs:grantTp', src)
end
exports('markTeleport', function(src) Aeigs.markTeleport(src) end)
AddEventHandler('aeigs:markTeleport', function(src) Aeigs.markTeleport(src or source) end)

function Aeigs.markRevive(src)
  src = tonumber(src)
  if not src or src <= 0 or not GetPlayerName(src) then return end
  TriggerClientEvent('aeigs:grantRevive', src)
  TriggerEvent('aeigs:revived', src)
end
exports('markRevive', function(src) Aeigs.markRevive(src) end)
...
local function heartbeat()
  Aeigs.request('/heartbeat', 'POST', {
    ...
    if ok and data and data.config then
      ...
      TriggerClientEvent('aeigs:protectedEvents', -1, ServerConfig.protectedEvents or {})
      -- Tam CoreAC config (panel Configuration sayfası) — CoreAC.Config'e uygula.
      if ServerConfig.ac and Aeigs.applyAcConfig then
        Aeigs.applyAcConfig(ServerConfig.ac)
      end
    end
  end)
end
...
RegisterNetEvent('aeigs:requestRules', function()
  ...
  TriggerClientEvent('aeigs:protectedEvents', source, ServerConfig.protectedEvents or {})
end)
...
CreateThread(function()
  ...
  for _, c in ipairs(candidates) do
    ...
    if raw and #raw > 0 then
      ...
      print(('^2[Core Shield] Logo loaded (%s, %d KB) — embedded in the connection card.^7'):format(c[1], math.floor(#raw / 1024)))
      break
    end
  end
end)
...
AddEventHandler('playerConnecting', function(name, setKickReason, deferrals)
  ...
  if ban then
    ...
    Aeigs.log('WARN', 'connect', ('Blocked banned connection: %s (Ban ID: %s)'):format(name, ban.code or '-'))
    return
  end
  ...
  do
    ...
    Aeigs.request('/network/check', 'POST', {
    ...
    if netResp and netResp.flagged and netResp.action == 'KICK' then
      ...
      Aeigs.log('WARN', 'connect', ('Network-blocked connection: %s (owners: %s)')
  ...
  deferrals.done()
  Aeigs.log('INFO', 'connect', ('%s is connecting'):format(name))
...
AddEventHandler('playerDropped', function(reason)
  local src = source
  Aeigs.log('INFO', 'disconnect', ('%s left (%s)'):format(GetPlayerName(src) or src, reason or ''))
  ...
  if a.type == 'WARN' then
    if src then
      TriggerClientEvent('chat:addMessage', src, {
        color = { 255, 200, 0 },
        args = { '[Core Shield]', ('Warning: %s'):format(a.reason or '') },
      })
    end
  elseif a.type == 'KICK' then
    if src then DropPlayer(src, '[Core Shield] You have been kicked from this server.') end
  elseif a.type == 'BAN' then
    if src then
      DropPlayer(src, ('[Core Shield] You are banned from this server. | Ban ID: %s'):format(a.banCode or '—'))
    end
    refreshBans()
...
local function fireScreenshotBurst(src, ids, onDone)
  ...
  CreateThread(function()
    for _, rid in ipairs(ids) do
      ...
      if Aeigs.issueShot then Aeigs.issueShot(rid, src) end
      TriggerClientEvent('aeigs:screenshot', src, base .. '?rid=' .. rid, rid, nil)
      Wait(400)
    end
    onDone()
  end)
    ...
    if data.banned then
      fireScreenshotBurst(src, data.screenshotRequestIds, function()
        if GetPlayerName(src) then
          DropPlayer(src, ('[Core Shield] You are banned from this server. | Ban ID: %s'):format(data.banCode or '—'))
        end
      end)
      refreshBans()
    elseif data.kicked then
      DropPlayer(src, '[Core Shield] You have been kicked from this server.')
    end
  end)
end)
...
CreateThread(function()
  if not Config.Token or Config.Token == '' then
    print('^1[Core Shield] WARNING: aeigs_token is not set. Add your server token to server.cfg.^7')
    return
  end
  if CoreAC and CoreAC.drawLogo then CoreAC:drawLogo() end
  print('^2[Core Shield] Anti-cheat started. Connecting to the panel...^7')
  if Config.DetectionsEnabled == false then
    print('^3[Core Shield] NOTE: detections are currently OFF. Enable them with set aeigs_detections "true".^7')
  end
  refreshBans()
  heartbeat()
  syncResources()
