...
--
-- Client core.lua her ~10 sn 'aeigs:alive' gönderir. Bir hileci client AC'yi
...
--   * SADECE en az bir kez 'aeigs:alive' GÖNDERMİŞ oyuncu izlenir (yükleme/
...
RegisterNetEvent('aeigs:alive', function()
  local src = source
...
-- ---------------------------------------------------------------------------
-- CHALLENGE-RESPONSE — "aeigs:alive" taklidini kapatır.
--
-- Pasif heartbeat tek başına yetersizdi: AC'yi kapatan hileci kendi kodundan
-- TriggerServerEvent('aeigs:alive') spam'leyip canlı görünebiliyordu. Artık
...
RegisterNetEvent('aeigs:challengeReply', function(nonce, answer)
  local src = source
  local p = pending[src]
        ...
        if s and GetPlayerName(src) and (now - s.first) > WARMUP then
          ...
          if ping and ping > 0 and ping < MAX_PING
            ...
            if (fails[src] or 0) >= FAIL_LIMIT then
              fails[src] = 0
              TriggerEvent('aeigs:serverReport', src, 'AC_TAMPER', 'HIGH', {
                reason = 'anti-tamper challenge failed (client anti-cheat not responding correctly)',
                failures = FAIL_LIMIT,
              })
              Aeigs.log('WARN', 'anticheat', ('AC_TAMPER: %s (challenge failed x%d)')
            ...
            elseif not pending[src] then
              ...
              TriggerClientEvent('aeigs:challenge', src, nonce)
            end
          end
        end
      end
        ...
        if s and GetPlayerName(src) then
          ...
          if ping and ping > 0 and ping < MAX_PING then
            if (now - s.first) > WARMUP
              ...
              if not (Aeigs.isWhitelisted and Aeigs.isWhitelisted(src)) then
                s.flagged = true
                TriggerEvent('aeigs:serverReport', src, 'AC_TAMPER', 'HIGH', {
                  reason = 'client anti-cheat heartbeat stopped (resource stopped or suppressed)',
                  silentFor = math.floor((now - s.last) / 1000) .. 's',
                })
                Aeigs.log('WARN', 'anticheat', ('AC_TAMPER: %s (no heartbeat for %ss)')
